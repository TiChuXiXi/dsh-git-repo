window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GitBody(props) {
      ...
      /** 某个提交的右键菜单项：按功能分组（查看 / 复制 / 分支 / 远程 / 修改历史），子菜单递归渲染。 */
      function commitMenuItems(commit) {
        ...
        // push 默认关闭（allowPush=false），与工具栏 Push 同一道门禁；配置没读到之前一律置灰。
        const allowPush = config !== null && config.allowPush === true
        const pushTitle = config === null
          ? '插件配置还没读到（等首次快照完成）'
          : 'push 已被插件配置关闭（allowPush=false）'
        const branchName = repo !== null && typeof repo.branch === 'string' ? repo.branch : ''
        const upstream = repo !== null && typeof repo.upstream === 'string' ? repo.upstream : ''
        const detached = repo !== null && repo.detached === true
        const items = [
          { kind: 'group', label: '查看' },
          {
            id: 'detail',
          ...
          { kind: 'group', label: '远程' },
          {
            id: 'push',
            label: '推送当前分支（Push）',
            hint: allowPush === true && upstream !== '' ? upstream : undefined,
            title: allowPush === true
              ? (upstream === '' ? `当前分支 ${branchName === '' ? '（未知）' : branchName} 还没有 upstream，先抓取或用「推送并设置 upstream」` : `git push → ${upstream}`)
              : pushTitle,
            disabled: busy === true || allowPush !== true || detached === true,
            onPick: () => run('push', {}, 'Push 中…'),
          },
          // 没有 upstream 时（新分支第一次推）额外给一条 -u 的入口，免得手动去终端设跟踪关系。
          upstream === '' && detached === false && branchName !== '' ? {
            id: 'push-set-upstream',
            label: '推送并设置 upstream',
            hint: `origin/${branchName}`,
            title: allowPush === true ? `git push --set-upstream origin ${branchName}` : pushTitle,
            disabled: busy === true || allowPush !== true,
            onPick: () => run('push', { setUpstream: true, branch: branchName }, '推送并设置 upstream 中…'),
          } : null,
        ...
        return items.filter((entry) => entry !== null && entry !== undefined)
      }
