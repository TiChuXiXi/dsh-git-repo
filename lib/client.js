window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      tr: { display: 'flex', alignItems: 'stretch', boxSizing: 'border-box' },
      ...
      tdGraph: { display: 'flex', alignItems: 'stretch', gap: '4px', padding: '5px 6px 5px 0', overflow: 'hidden', boxSizing: 'border-box' },
      lane: { position: 'relative', flex: 'none', width: '2px', marginLeft: '12px' },
      graphLine: { position: 'absolute', left: 0, top: 0, bottom: 0, width: '2px', background: 'rgba(59,130,246,0.35)' },
      node: {
        position: 'absolute', left: '-4px', top: '9px', width: '9px', height: '9px',
        borderRadius: '50%', border: '2px solid #3b82f6', background: 'transparent', boxSizing: 'border-box',
      },
      nodeHead: { background: '#3b82f6' },
      chips: { display: 'flex', flex: 1, minWidth: 0, gap: '3px', flexWrap: 'wrap', alignContent: 'flex-start', paddingTop: '2px' },
      chipBranch: {
        flex: 'none', fontSize: '10px', lineHeight: '15px', padding: '0 5px', borderRadius: '999px',
        border: '1px solid rgba(128,128,128,0.45)', background: 'rgba(128,128,128,0.1)', whiteSpace: 'nowrap',
      },
      chipLocal: { borderColor: 'rgba(34,197,94,0.55)', background: 'rgba(34,197,94,0.12)' },
      chipRemote: { borderColor: 'rgba(59,130,246,0.5)', background: 'rgba(59,130,246,0.1)' },
      chipHead: { borderColor: 'rgba(34,197,94,0.95)', background: 'rgba(34,197,94,0.3)', fontWeight: 700 },
      ...
      notice: { flex: 'none', padding: '6px 8px', background: 'rgba(34,197,94,0.12)', fontSize: '11px' },
      noticeBad: { flex: 'none', padding: '6px 8px', background: 'rgba(239,68,68,0.14)', fontSize: '11px' },
      ...
      title: { display: 'inline-flex', alignItems: 'center', gap: '5px', whiteSpace: 'nowrap', flex: 'none', minWidth: '78px', overflow: 'hidden' },
    }
    ...
    /**
     * Log 的提交树列：左侧一条泳道线 + 节点，节点右侧是**所有指向该提交的分支标签**
     * （本地与远程都在这里标出来，一眼能看出每个分支停在哪个提交）。
     * 列宽由 GitBody 依标签内容算好传进来（自适应，且不提供拖动手柄）。
     */
    function GraphCell(props) {
      const labels = Array.isArray(props.labels) ? props.labels : []
      const line = { ...S.graphLine }
      if (props.first === true && props.last === true) line.display = 'none'
      else if (props.first === true) line.top = '13px'
      else if (props.last === true) line.height = '13px'
      const node = props.first === true ? { ...S.node, ...S.nodeHead } : S.node
      ...
      return h('div', {
        ...
        h('div', { style: S.lane }, h('div', { style: line }), h('div', { style: node })),
        h('div', { style: S.chips }, labels.map((label) => h('span', {
          key: `${label.kind}:${label.name}`,
          style: label.head === true
            ? { ...S.chipBranch, ...S.chipLocal, ...S.chipHead }
            : (label.kind === 'local' ? { ...S.chipBranch, ...S.chipLocal } : { ...S.chipBranch, ...S.chipRemote }),
    ...
    function GitBody(props) {
      ...
      useEffect(() => {
        ...
        rpc('diff', { path: selected.path, staged: selected.staged === true }).then((result) => {
          if (!live) return
          setPatchLoading(false)
      ...
      function renderChanges() {
        ...
        return h('div', { style: S.body },
              ...
              return h('div', { key: group.id },
                ...
                list.map((entry) => {
                  ...
                  return h('div', {
                    ...
                    title: entry.path,
                    onClick: () => setSelected({ path: entry.path, staged: group.id === 'staged', untracked: entry.untracked === true }),
                  },
          ...
          selected === null ? null : h('div', { style: S.detail },
            h('div', { style: S.bar },
              h('span', { style: S.name, title: selected.path }, `差异 · ${selected.path}${selected.staged === true ? '（已暂存）' : '（工作区）'}`),
              h('span', { style: S.spacer }),
      ...
      function renderCommitBody(commit) {
        ...
        const body = shown === null || typeof shown.patch !== 'string' ? '' : shown.patch
        const allowDangerous = config === null || config.allowDangerous !== false
        return h('div', { style: S.bodyCol },
          h('div', { style: S.head }, `${commit.shortHash} · ${commit.author} · ${shortDate(commit.date)}`),
          h('div', { style: S.bar },
          ...
          h('div', { style: S.head }, commit.subject),
          commit.body === '' || commit.body === undefined ? null : h('pre', { style: S.pre }, commit.body),
          h('div', { style: S.head }, `变更文件 · ${files.length}`),
          files.map((file) => h('div', { key: file.path, style: S.row, title: file.path },
          ...
          h('div', { style: S.head }, 'Patch'),
          DiffView({ text: body, loading: false }),
        )
      }
        ...
        return h('div', { style: S.body },
          h('div', { style: S.col },
            ...
            commits.length === 0
              ...
              : commits.map((item, index) => h('div', {
                key: item.hash,
                style: activeHash !== null && activeHash === item.hash ? { ...rowStyle, ...S.rowOn } : rowStyle,
                title: item.subject,
                onClick: () => openCommit(item.hash),
              },
                cell('date', logWidths.date, shortDate(item.date), item.date),
                h(GraphCell, { key: 'graph', first: index === 0, last: index === last, labels: logLabelsOf(tips, item), width: graphWidth }),
      ...
      return h('div', { style: S.root },
        ...
        notice === null ? null : h('div', { style: notice.bad === true ? S.noticeBad : S.notice }, notice.text),
