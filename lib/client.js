window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      table: { tableLayout: 'fixed', width: '100%', borderCollapse: 'collapse', fontSize: '12px' },
      th: {
        textAlign: 'left', padding: '4px 8px', color: MUTED, fontWeight: 500, borderBottom: BORDER,
        position: 'sticky', top: 0, background: 'var(--dsw-alias-bg-layer-1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
      },
      td: { padding: '5px 8px', verticalAlign: 'top', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
      tdGraph: { position: 'relative', padding: 0 },
      graphLine: { position: 'absolute', left: '50%', width: '2px', marginLeft: '-1px', top: 0, bottom: 0, background: 'rgba(59,130,246,0.35)' },
      node: {
    ...
    /** Log 的图形列：一条竖线 + 一个节点；首行线只向下，末行线只向上。 */
    function GraphCell(props) {
      const line = { ...S.graphLine }
      if (props.first === true) line.top = '50%'
      if (props.last === true) line.bottom = '50%'
      const node = props.first === true ? { ...S.node, ...S.nodeHead } : S.node
      return h('td', { style: S.tdGraph }, h('div', { style: line }), h('div', { style: node }))
    }
    ...
    function GitBody(props) {
      ...
      function renderLogDetail() {
        ...
        return h('div', { style: S.detail },
          h('div', { style: S.bar },
            h('span', { style: S.meta }, commit === null ? '提交详情' : commit.hash),
            h('span', { style: S.spacer }),
          ...
          h('div', { style: S.col },
            h('table', { style: S.table },
              h('colgroup', null,
                ...
                h('col', { style: { width: '74px' } }),
                h('col', null),
              ),
              h('thead', null, h('tr', null,
                h('th', { style: S.th }, ''),
                h('th', { style: S.th }, 'Author'),
                h('th', { style: S.th }, 'Date'),
                h('th', { style: S.th }, 'Commit'),
                h('th', { style: S.th }, 'Message'),
              )),
              h('tbody', null, commits.map((item, index) => h('tr', {
                key: item.hash,
                style: activeHash !== null && activeHash === item.hash ? S.rowOn : undefined,
                title: item.subject,
                onClick: () => openCommit(item.hash),
              },
                GraphCell({ first: index === 0, last: index === last }),
                h('td', { style: S.td, title: item.author }, item.author),
                h('td', { style: S.td, title: item.date }, shortDate(item.date)),
                h('td', { style: { ...S.td, ...S.meta }, title: item.hash }, item.shortHash),
                h('td', { style: S.td, title: item.subject }, item.subject),
              ))),
            ),
          ),
          renderLogDetail(),
      ...
      function renderRemotes() {
        ...
        if (list.length === 0) return h('div', { style: S.placeholder }, '没有配置任何远程仓库（git remote -v 为空）')
        return h('div', { style: S.bodyCol },
          h('div', { style: S.head }, `远程仓库 · ${list.length}`),
      ...
      return h('div', { style: S.root },
        ...
        notice === null ? null : h('div', { style: S.notice }, notice),
