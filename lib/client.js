window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      /* 圆点：空心，居中于本行，圆环用该分支色系的深色。 */
      node: {
        position: 'absolute', left: '-4px', top: '50%', marginTop: '-5px', width: '10px', height: '10px',
        borderRadius: '50%', border: '2px solid #2563eb', background: 'transparent', boxSizing: 'border-box',
      },
      /* HEAD 的空心圆用更粗的圆环区分（不用实心填充，避免遮住连线）。 */
      nodeHead: { borderWidth: '3px' },
      chips: {
    ...
    /**
     * 提交树调色板：ring = 同色系深色（空心圆环 / 标签边框），line = 同色系浅色（连线），
     * fill = 更浅的标签底色。按分支名排序取模分配，保证跨渲染稳定。
     */
    const GRAPH_COLORS = [
      { ring: '#2563eb', line: 'rgba(37,99,235,0.42)', fill: 'rgba(37,99,235,0.14)' },
      { ring: '#16a34a', line: 'rgba(22,163,74,0.42)', fill: 'rgba(22,163,74,0.14)' },
      { ring: '#7c3aed', line: 'rgba(124,58,237,0.42)', fill: 'rgba(124,58,237,0.14)' },
      { ring: '#ea580c', line: 'rgba(234,88,12,0.42)', fill: 'rgba(234,88,12,0.14)' },
      { ring: '#db2777', line: 'rgba(219,39,119,0.42)', fill: 'rgba(219,39,119,0.14)' },
      { ring: '#0d9488', line: 'rgba(13,148,136,0.42)', fill: 'rgba(13,148,136,0.14)' },
      { ring: '#ca8a04', line: 'rgba(202,138,4,0.42)', fill: 'rgba(202,138,4,0.14)' },
      { ring: '#4f46e5', line: 'rgba(79,70,229,0.42)', fill: 'rgba(79,70,229,0.14)' },
    ]
    ...
    /**
     * Log 的提交树列：一条贯穿整行的泳道 + 本行正中的**空心圆点**，圆点右侧是该提交上的分支标签。
     *
     * 连线分上下两段，分别止于圆环外沿（圆点内部不画线），段与段跨行首尾相接所以看不出接缝；
     * 圆环用该分支色系的**深色**，连线用同色系**浅色**，不同分支不同色；HEAD 的空心圆环加粗。
    ...
    function GraphCell(props) {
      ...
      // 两段各向相邻行越界 1px：行高出现小数时也能与上下行的段严丝合缝（同色重叠看不出来）。
      ...
      const node = props.first === true ? { ...S.node, ...S.nodeHead, borderColor: ring } : { ...S.node, borderColor: ring }
    ...
    function GitBody(props) {
      ...
      /**
       * 提交 → 调色板下标：本行有分支标签就用第一个标签（HEAD / 本地优先）的颜色，
       * 否则从子提交沿父提交往下继承（列表是新→旧，子提交先处理），让一条线从头到尾同色。
       */
      function logColorOf(tips, colorByName) {
        const colorOf = new Map()
        const inherited = new Map()
        for (const item of commits) {
          const labels = logLabelsOf(tips, item)
          const labeled = labels.length === 0 ? undefined : colorByName.get(labels[0].name)
          const index = labeled !== undefined ? labeled : (inherited.has(item.hash) === true ? inherited.get(item.hash) : 0)
          colorOf.set(item.hash, index % GRAPH_COLORS.length)
          for (const parent of item.parents) {
            if (inherited.has(parent) === false) inherited.set(parent, index)
          }
        }
        return colorOf
      }
      ...
      function renderLog() {
        ...
        const colorOf = logColorOf(tips, colorByName)
