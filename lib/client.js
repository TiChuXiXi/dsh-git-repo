window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const S = {
      ...
      check: {
        flex: 'none', width: '13px', height: '13px', boxSizing: 'border-box', borderRadius: '3px', border: BORDER,
      ...
      checkOn: { background: 'rgba(59,130,246,0.9)', borderColor: 'rgba(59,130,246,0.9)', color: '#fff' },
      checkOff: { background: 'transparent', color: 'transparent' },
      checkDim: { opacity: 0.35, cursor: 'default' },
    ...
    function GitBody(props) {
      ...
      async function commit() {
        // 只提交勾选的文件：host 的 commit 带 pathspec 时只提这些路径的工作区内容。
